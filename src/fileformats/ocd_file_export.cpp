		...
		
	{
		...
		for (int i = 0; i < num_colors; ++i)
		{
			...
			if (color->getOpacity() < 1)
				addWarning(::LibreMapper::OcdFileExport::tr("Variable color opacity in \"%1\" is not supported by OCD version 8. Exporting as a fully opaque color.")
				           .arg(color->getName()));
			const auto& cmyk = color->getCmyk();
...
QByteArray OcdFileExport::exportObjectCommon(const Object* object, OcdObject& ocd_object, typename OcdObject::IndexEntryType& entry)
{
	...
	if (ocd_version == 8)
	{
		...
		if (entry_size > 32767)
			throw FileFormatException(::LibreMapper::OcdFileExport::tr("The map contains an object with more than 32767 coordinates which is not supported by OCD version 8."));
