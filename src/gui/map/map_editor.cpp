...
void MapEditorController::selectSizeClicked()
{
	...
	select_objects_dialog->setWindowTitle(tr("Select objects by their area size"));
	auto* form_layout = new QVBoxLayout(select_objects_dialog);

	auto* object_size_widget = new QWidget(select_objects_dialog);
	auto* size_input_layout = new QFormLayout(object_size_widget);
	auto* object_size_label = new QLabel(tr("Object size (mm²)"), object_size_widget);
	size_input_layout->setWidget(0, QFormLayout::LabelRole, object_size_label);
	auto* object_size_input = new QDoubleSpinBox(object_size_widget);
	size_input_layout->setWidget(0, QFormLayout::FieldRole, object_size_input);
	...
	if (active_symbol && active_symbol->getType() == Symbol::Area)
	{
	...
	for (auto* object : map->selectedObjects())
	{
		...
		auto object_size = object->asPath()->parts().front().calculateArea();
...
void MapEditorController::enlargeClicked()
{
	...
	for (auto* object : map->selectedObjects())
	{
		if (object->getSymbol()->getContainedTypes() & (Symbol::Line | Symbol::Area))
		{
			...
			qreal object_size = -1;
			qreal desired_size = -1;

			const PathPartVector& parts = path->parts();
			if (symbol->getType() == Symbol::Area)
			{
				desired_size = symbol->asArea()->getMinimumArea(); // getMinimumArea() returns thousands of square millimeters
				object_size = parts.front().calculateArea() * 1000; // millimeters, multiply by 1000 to match the above unit
			}
			...
			if (desired_size > object_size && object_size > 0)
			{
				...
				// We rescale area objects only hence the square root in the scaling factor calculation.
				object->scale(MapCoordF(object->getExtent().center()), std::sqrt(desired_size) / std::sqrt(object_size));
				object->update();
			}
		}
	}
