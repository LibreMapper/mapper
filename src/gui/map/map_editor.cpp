...
void MapEditorController::enlargeClicked()
{
	...
	for (auto* object : map->selectedObjects())
	{
		if (object->getSymbol()->getContainedTypes() & (Symbol::Line | Symbol::Area))
		{
			...
			{
				auto const desired_size = symbol->asArea()->getMinimumArea(); // getMinimumArea() returns thousands of square millimeters
			...
			{
				auto const desired_size = symbol->asLine()->getMinimumLength(); // micrometers
