...
private slots: // NOLINT(readability-redundant-access-specifiers)
	...
	void testBooleanTool() // NOLINT(readability-function-cognitive-complexity)
	{
		...
		auto obj_comparator = [](auto const* a_ptr, auto const* b_ptr) {
			...
			{
				auto const p = a->getCoordinate(i);
				auto const q = b->getCoordinate(i);
				// We allow small differences in the resulting coordinates as
				// rounding errors vary across target platforms
				if (p.flags() != q.flags() || (p != q && p.distanceSquaredTo(q) > 2))
					return false;
			}
			return true;
		};
